...
public class ScoreInformation {
    ...
    public void increment(String user) {
        ...
        long points = Duration.between(contestState.getRoundTime(), Instant.now()).toSeconds();
        
        // to avoid negative points
        points = Math.max(points, 0);
        long timeBetweenQuestions = contestState.getCurrentContest().get().timeBetweenRounds.getSeconds();
